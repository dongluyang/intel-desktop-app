...
export default {
  ...
  methods: {
    ...
    getDimensions () {
      ...
      console.log(JSON.stringify(dimensions) + 'offset' + this.container.offsetWidth)
    ...
    resetPicture () {
      ...
      // console.log(JSON.stringify(dimensions)+"reset")
      ...
      if (this.isGif && this.isAvailable && !this.isMovie) {
        ...
        this.picturePath = await getImageByUrl(this.preview.url)
      }
    ...
    setPictureDlPath () {
      if (this.preview && this.isAvailable && !this.isMovie) {
        this.pictureDlPath = this.preview.url
      } else {
