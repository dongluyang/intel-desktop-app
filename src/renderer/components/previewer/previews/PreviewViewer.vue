...
import PictureViewer from '../previews/PictureViewer.vue'
import Spinner from '../widgets/Spinner.vue'
import VideoViewer from '../previews/VideoViewer.vue'

import featCompt from '../lib/feathercompt';
import {getObsUrl, getSmallFile} from "../../../utils/api";
featCompt(DownloadIcon);
...
export default {
  ...
  computed: {
    ...
    isMovie () {
      return this.isReady && (this.extension === 'mp4'||this.extension === 'mov')
    },
  ...
  watch: {
    preview () {
      this.refresh = false
			if (this.preview.storage ==='GridFS') {
				getSmallFile(this.preview.id).then(data => {
					var ext = this.preview.name.substring(this.preview.name.lastIndexOf('.') + 1)
					//to do temporarily
					if (ext==='mov' || ext ==='mp4') {
						ext = 'mp4'
						this.preview.type = 'video'
					}
					this.preview.extension = ext
					this.preview.url = 'data:' + this.preview.type + "/" + ext + ";base64," + data.content
					this.refresh = true
					if (this.isMovie) {
						this.pause()
						this.maxDuration = '00:00.000'
			...
			} else if(this.preview.storage ==='Huawei_OBS'){
				getObsUrl(this.preview.name,this.preview.taskName,this.preview.projectName).then(data => {
					var ext = this.preview.name.substring(this.preview.name.lastIndexOf('.') + 1)
					//to do temporarily
					if (ext==='mov' || ext ==='mp4') {
						ext = 'mp4'
