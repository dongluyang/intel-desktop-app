...
import {getImageByUrl} from "../../myCheckin/review";
...
export default {
  ...
  computed: {
    ...
    moviePath () {
      if (this.isMovie && this.isAvailable && !this.isHd) {
        return this.preview.url
      } else if (this.isMovie && this.isAvailable) {
        return this.preview.url
      } else {
        return null
      }
    ...
    posterPath () {
      if (this.isMovie && this.isAvailable) {
        return 'https://cgyun-app-image.oss-cn-zhangjiakou.aliyuncs.com/v7.png'
      } else {
        return null
      }
    },
  ...
  methods: {
    ...
    getNaturalDimensions () {
      return {
    	...
    	console.log(this.preview)
			this.preview.url = await getImageByUrl(this.preview.url)
		},
    ...
    getDimensions () {
      ...
      let width = Math.max(dimensions.width, offsetWidth)
      if (this.isComparing) {
    ...
    setCurrentTime (currentTime) {
      ...
      console.log("<>"+this.$options.running+"<>"+currentTime+"<>"+this.$options.currentTimeCalls.length)
    ...
    goNextFrame () {
      const time = this.getLastPushedCurrentTime()
      const newTime = roundToFrame(time, this.fps) + this.frameFactor
      console.log(time+"<>"+newTime+"<>"+this.video.duration)
