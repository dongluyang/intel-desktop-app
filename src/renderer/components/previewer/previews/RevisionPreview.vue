 ...
 */
import LightEntityThumbnail from '../widgets/LightEntityThumbnail.vue'
import {getSmallFile} from "../../../utils/api";
...
export default {
  ...
  mounted () {
    ...
    if (this.previewFile!=null && this.previewFile.url ==undefined && ['image','jpg', 'png','jpeg'].includes(this.previewFile.type)) {
          getSmallFile(this.previewFile.id).then(data => {
              var ext = this.previewFile.name.substring(this.previewFile.name.lastIndexOf('.') + 1)
              this.previewFile.extension = ext
              this.previewFile.url = 'data:image/' + ext + ";base64," + data.content
          });
    }
