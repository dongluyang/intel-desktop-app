...
// 鼠标在画布上按下
    rectMouseDown(e) {
        if (!this.isObjectSelected && this.currentType!=null) {

          if (this.downPoint !=null) {
            this.fabricCanvas.selection = true
            if ('rect' === this.currentType && this.rect!=null) {
              this.fabricCanvas.add(this.rect)
              this.addToUpdates(this.rect)
              this.saveAnnotations()
            } else if ('arrow' === this.currentType) {
              this.createArrow(e.absolutePointer)
            }
            this.downPoint = null
            return
          }
    ...
    loadSingleAnnotation (annotation) {
      ...
      annotation.drawing.objects.forEach(obj => {
        ...
        if (obj.type === 'path') {
          ...
          const polyline = new fabric.Polyline(obj.points,
            {
              ...base
            }
          )
          polyline.setControlsVisibility({
            mt: false,
            mb: false,
            ml: false,
            mr: false,
    ...
    configureCanvas () {
      this.fabricCanvas.off('object:moving', this.onObjectMoved)
      ...
      this.fabricCanvas.off('mouse:move', this.rectMouseMove)
      this.fabricCanvas.off('mouse:down', this.onCanvasClicked)
      this.fabricCanvas.off('mouse:down',this.rectMouseDown)
      this.fabricCanvas.off('selection:created',this.onObjectSelect)
      this.fabricCanvas.off('selection:cleared',this.onObjectClear)
      this.fabricCanvas.on('object:moving', this.onObjectMoved)
      ...
      this.fabricCanvas.on('mouse:move', this.rectMouseMove)
      this.fabricCanvas.on('mouse:down', this.onCanvasClicked)
      this.fabricCanvas.on('mouse:down',this.rectMouseDown)
      this.fabricCanvas.on('mouse:up', this.onCanvasReleased)
      this.fabricCanvas.on('selection:created',this.onObjectSelect)
      this.fabricCanvas.on('selection:cleared',this.onObjectClear)
      this.fabricCanvas.freeDrawingBrush.color = this.color
    ...
    endDrawing () {
      if (this.isDrawing || this.isSquare || this.isArrow) {
    ...
    createRect(pointer) {
      // 点击事件，不生成矩形
      if (this.downPoint == null || this.rect == null) {
        return
      }
      // 创建矩形
      // 矩形参数计算
      ...
      // 矩形对象
      this.rect.set('top', top).set('left', left).set('width', width).set('height', height);
    ...
    initRect() {
      // 创建矩形
      // 矩形参数计算
      let top = this.downPoint.y
      let left = this.downPoint.x
      let width = 0
      let height = 0
      // 矩形对象
    ...
    createArrow(pointer) {
      ...
      var toy = pointer.y
      var tox = pointer.x
      var fromy  = this.downPoint.y
      var fromx = this.downPoint.x
      ...
      this.pline = new fabric.Polyline(points, {
      ...
      this.fabricCanvas.add(this.pline)
